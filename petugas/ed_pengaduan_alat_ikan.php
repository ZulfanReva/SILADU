...
if (isset($_POST['simpan'])) {
    if (!empty($jenis_alat) && !empty($waktu_kerusakan) && !empty($penyebab_kerusakan) && !empty($permintaan) && !empty($tgl_selesai)) {
        ...
        // Upload gambar keterangan petugas
        ...
        if ($_GET['hal'] == "edit") {
            ...
            if ($edit) {
                echo "<script>
                    ...
                    window.location.href = 'hasil_pengaduan_alat_ikan.php';
                </script>";
            } else {
                echo "<script>alert('Edit Data Gagal!'); history.back();</script>";
            }
        } else {
            $sql = "INSERT INTO pengaduan_alat_perikanan ( jenis_alat, waktu_kerusakan, penyebab_kerusakan,  path_gambar, tgl_selesai, catatan_admin, keterangan_petugas, tgl_pengaduan)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";

            $stmt = mysqli_prepare($koneksi, $sql);
            mysqli_stmt_bind_param($stmt, "ssssssss", $jenis_alat, $waktu_kerusakan, $penyebab_kerusakan, $tgl_selesai, $catatan_admin, $gambar_baru, $gambar_petugas_baru);
