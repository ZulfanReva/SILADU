...
if (isset($_POST['simpan'])) {
    ...
    echo "<script>location('hasil_pengaduan_alat_ikan.php');</script>";
}
...
if (isset($_GET['hal'])) {
    if ($_GET['hal'] == "hapus") {
        ...
        if ($hapus) {
            echo "<script>alert('Hapus Data Sukses!'); location='hasil_pengaduan_alat_ikan.php';</script>";
        } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['upload'])) {
    ...
    $target_dir = "../petugas/uploads/hasil_pengaduan_alat_ikan/";
    ...
    if (in_array($file_ext, $allowed_exts)) {
        ...
        if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
            ...
            try {
                ...
                $query = "UPDATE pengaduan_alat_perikanan 
                           SET keterangan_petugas = ? 
                           WHERE id_pengaduan = ?";
                $stmt = $koneksi->prepare($query);
                $stmt->bind_param("si", $new_file_name, $id_pengaduan);
                $stmt->execute();

                // Commit transaksi jika kedua update berhasil
                mysqli_commit($koneksi);

                echo "<script>alert('Gambar berhasil diupload dan disimpan!'); window.location.href='hasil_pengaduan_alat_ikan.php';</script>";
            } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT stts_pengaduan, tgl_selesai FROM pengaduan_alat_perikanan WHERE id_pengaduan = ?";
    $stmt = $koneksi->prepare($query);
    ...
    // Update data
    $sql = "UPDATE pengaduan_alat_perikanan SET stts_pengaduan = ?, tgl_selesai = ? WHERE id_pengaduan = ?";
    $stmt = $koneksi->prepare($sql);
    $stmt->bind_param("ssi", $stts_pengaduan, $tgl_selesai, $id_pengaduan);

    if ($stmt->execute()) {
        $stmt->close();
        echo "<script>alert('Data berhasil diperbarui!'); window.location.href='hasil_pengaduan_alat_ikan.php';</script>";
        exit;
    } else {
...
if (isset($_GET['hal'])) {
    if ($_GET['hal'] == "hapus") {
        ...
        $update = mysqli_query($koneksi, "UPDATE pengaduan_alat_perikanan SET stts_pengaduan='Disetujui' WHERE id_pengaduan='$_GET[id]'");
        if ($update) {
            echo "<script>alert('Pengajuan Disetujui!'); location='hasil_pengaduan_alat_ikan.php';</script>";
        } else {
            echo "<script>alert('Gagal Menyetujui!');</script>";
        }
    } elseif ($_GET['hal'] == "tolak") {
        $update = mysqli_query($koneksi, "UPDATE pengaduan_alat_perikanan SET stts_pengaduan='ditolak' WHERE id_pengaduan='$_GET[id]'");
        if ($update) {
            echo "<script>alert('Pengajuan Ditolak!'); location='hasil_pengaduan_alat_ikan.php';</script>";
        } else {
...
<body>
    ...
    <h1>Laporan Data Pengaduan Perikanan</h1>
                ...
                <table class="table table-bordered table-striped">
                    ...
                    if (!empty($search)) {
                        ...
                        if (strpos($query, "WHERE") !== false) {
                            $query .= " AND (user.nama LIKE '%$search%' OR pengaduan_alat_perikanan.jenis_alat LIKE '%$search%')";
                        } else {
                            $query .= " WHERE (user.nama LIKE '%$search%' OR pengaduan_alat_perikanan.jenis_alat LIKE '%$search%')";
                        }
                    }
                    ...
                    while ($row = mysqli_fetch_array($result)) :
                        $penggunaImagePath = "../petugas/uploads/pengaduan_alat_perikanan/" . $row['path_gambar'];
                        ...
                        <tr>
                            ...
                            <?php if ($_SESSION['level'] == 'admin' || $_SESSION['level'] == 'petugas') { ?>
                                <td class="text-center">
                                    <a href="ed_pengaduan_alat_ikan.php?hal=edit&id=<?php echo $row['id_pengaduan'] ?>" class="btn btn-warning"><i class="fa-solid fa-pen-to-square"></i></a>

                                    <a href="hasil_pengaduan_alat_ikan.php?hal=hapus&id=<?= $row['id_pengaduan'] ?>" onclick="return confirm('Apakah yakin ingin menghapus data ini?')" class="btn btn-danger btn-sm" title="Hapus Pengajuan">
                                        <i class="fa-solid fa-trash"></i> Hapus
                                    </a>

                                    <a href="hasil_pengaduan_alat_ikan.php?hal=setuju&id=<?= $row['id_pengaduan'] ?>" class="btn btn-success btn-sm" title="Setujui Pengajuan">
                                        <i class="fa-solid fa-check"></i> Setujui
                                    </a>

                                    <a href="hasil_pengaduan_alat_ikan.php?hal=tolak&id=<?= $row['id_pengaduan'] ?>" class="btn btn-warning btn-sm" title="Tolak Pengajuan">
    ...
    `;
            ...
            printWindow.document.write('<html><head><title>Cetak Hasil Laporan Pengaduan Alat Ikan</title>');
