...
if (!empty($search)) {
    $query .= " WHERE jenis_bantuan LIKE '%" . $koneksi->real_escape_string($search) . "%'";
}

$query .= " GROUP BY jenis_bantuan";
...
<head>
    <title>Dashboard - Rekap Bantuan</title>
...
<body>
    ...
    <!-- Rekap Pengaduan -->
    <div class="card-wrap">
        <h3 class="text-center">Rekap Permohonan Bantuan</h3>
        ...
        <form method="GET">
            <input type="text" name="search" placeholder="Jenis Bantuan" value="<?php echo htmlspecialchars($search); ?>" class="form-control d-inline-block" style="width: auto; display: inline-block; margin-right: 10px;">
        ...
        <div class="container mt-5">
            <h3 class="text-center">Grafik Rekap Permohonan bantuan</h3>
    ...
    <script>
        ...
        const rekapChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: <?= json_encode(array_column($rekapData, 'jenis_bantuan')); ?>,
                datasets: [
                    {
                        label: 'Disetujui',
    ...
    `;
            ...
            printWindow.document.write('<html><head><title>Cetak Laporan Rekap Permohonan Bantuan </title>');
